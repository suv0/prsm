  ...
  return [
    ...
    ...(context.extraInstructions?.trim()
      ? [
          ...
          "These come from the person who started this review. Follow them for this pass.",
          context.extraInstructions.trim(),
          "",
        ]
      : []),
    ...
    "- Sound like a teammate reading the code, not a scanner or formal finding.",
    "- Prefer thought-order when it fits: notice → simple question → concrete scenario → plain consequence → ask.",
    "- Vary openings. “Hm… interesting.” is optional tone, not a required opener — do not stamp it (or So…/Then…/Is there any way…) onto every comment.",
    "- Keep concrete names and simple stakes (“costing us twice”), not corporate abstractions.",
    "- Multi-sentence is fine for real risks. Tiny nits may still use “Could we…?”",
    "- No shame toward the author. Never write like “This implementation contains an unsafe X.”",
    "- Bad (compressed tool voice): “Could we avoid failing over on Unifonic timeouts / accepted but no MessageID? Those can mean the SMS already went out…”",
    "- Good (thinking out loud, one example — invent fresh wording each time): notice the risky timeout path, say the phone could get two OTPs / we pay twice, ask to only failover when the provider clearly said send failed.",
    "",
    "whyWeak / howToFix:",
    ...
    "- If betterCode names a new file/import path, howToFix + reviewComment must use that same path.",
    "- Bad: howToFix says “move to data/utils” while betterCode shows src/data/permission-matrix.ts.",
    "- Good: all three point at apps/operator/src/data/permission-matrix.ts (or @/data/permission-matrix).",
    "",
